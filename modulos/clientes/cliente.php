<? 
	...
	$cliente = mysql_fetch_assoc(mysql_query("select * from tclientes where idcliente = $idcliente"));
	$lat = $cliente['latitud'];
	...
	if($_POST['enviar']==1){
		...
		$lat = $_POST['txtLat'];
		$lng = $_POST['txtLng'];
		mysql_query("update tclientes set 
						nombre = '$nombre', 
						correo = '$correo', 
						telefono = '$telefono', 
						direccion = '$direccion', 
...
function manejarEventos(evento){
	...
	if(code==114){
		parent.seleccionarCliente('<? echo $idcliente;?>');
		parent.$.fancybox.close();
	}
}
...
function initialize() {
	...
	<? if($lat!=""&& $lng!=""){ ?>
    var latlng = new google.maps.LatLng<? echo "($lat,$lng)"; ?>;
	<? }else{ ?>
	...
	function ubicarDireccion() {
		...
		var marker = new google.maps.Marker({
			map: map,
			position: new google.maps.LatLng<? echo "($lat,$lng)"; ?>
		});
		//faltaba esta linea
		markersArray.push(marker);
	}
	...
	function codeAddress() {
    	var address = document.getElementById('txtDireccion').value + ' Ensenada, B.C. Mexico';
...
<div style="position: absolute; width: 330px; height: 32px; top: 259px; left: 16px; z-index: 2;">
    <input type="text" name="txtDireccion" id="txtDireccion" class="campo" style="width: 330px; height: 32px;" value="<? echo strtoupper($cliente['direccion']); ?>" onblur="codeAddress();" />
  </div>
