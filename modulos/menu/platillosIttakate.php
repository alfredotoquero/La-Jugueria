...
<div style="position:relative;">
    ...
    <div style="position:absolute; width:580px; height:340px; top:60px; left:10px; overflow:auto;">
        ...
        <input type="hidden" name="enviarITTA" value="1" />
        <input type="hidden" name="idittakate" value="<? echo $idittakate;?>" />
        <table width="540" align="center" border="0" cellpadding="3" cellspacing="0">
			<?
            $ittakate=mysql_fetch_assoc(mysql_query("select * from tittakates where idittakate = '$idittakate'"));
            ?>
        	<tr bgcolor="#FFFFFF" height="30">
            	<td><b><? echo $ittakate["nombre"];?></b></td>
            </tr>
         	<?
			$categoria=1;
			while($categoria<6){
				...
				?>
					<tr><td style="color:#FFF;"><input type="radio" name="txtCategoria<? echo $categoria;?>" <? if($num==1){ ?>checked="checked"<? } ?> value="<? echo $categoria."-".$platillo['idplatillo']; ?>" /><? echo $platillo['platillo']; ?></td></tr>
				<?
					$num++;
				}
				$categoria++;
			...
			?>
            <tr>
            	<td align="center"><input type="button" name="btnGuardar" value="Guardar" onclick="parent.fancy(306,130,'modulos/menu/cantidad.php?' + $('#formITTA').serialize());"</td>
            </tr>
        </table>
        </form>
    </div>
